...
class WxTplmsgModel extends Model
{
    ...
    public function addWxTplMsg($data)
    {
        foreach ($data as $key => $value) {
            $tplmsg = $this->get(array("template_id_short" => $key));
            if ($tplmsg) {
                $tplmsg["template_id"] = $value;
                $this->save($tplmsg);
            } else {
                $this->add(array("template_id_short" => $key, "template_id" => $value));
            }
        }
