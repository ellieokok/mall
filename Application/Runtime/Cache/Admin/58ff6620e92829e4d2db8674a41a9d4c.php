<?php if (!defined('THINK_PATH')) exit();?><iframe src="http://addon.wemallshop.com/Home/addon/" allowtransparency="true" id="myiframe"
...
<script type="text/javascript">
    ...
    function install(path, filename, type) {
        $.ajax({
            type: "post",
            url: "<?php echo U('Admin/Addon/getFileDownload');?>",
            data: {
                path: path,
                filename: filename,
                type: type
    ...
    function compare(path, filename, filesize) {
        var interval = setInterval(function () {
            $.ajax({
                type: "post",
                url: "<?php echo U('Admin/Addon/compare');?>",
                data: {
                    path: path,
                    filename: filename,
                    filesize: filesize,
