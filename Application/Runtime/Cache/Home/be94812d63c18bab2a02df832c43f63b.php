    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="box box-default">
                ...
                <!-- form start -->
                <form class="form-horizontal" action="<?php echo U('Home/Order/search');?>" method="post">
                    <div class="box-body">
            ...
            <div class="box box-danger">
                ...
                <div class="box-body no-padding">
                    <div class="mailbox-controls">
                        <div class="btn-group">
                            <a href="<?php echo U('Home/Order/export',array('status'=>I('get.status') , 'pay_status'=>I('get.pay_status') ,'day'=>I('get.day')));?>"
                    ...
                    <div class="mailbox-controls">
                        <div class="btn-group">
                            <button type="button" class="btn btn-danger "
                                    onclick="batchUrl('<?php echo U('Home/Order/wxPrint');?>')">打印
                            </button>
                        </div>
                        <div class="btn-group">
                            <button type="button" class="btn btn-danger "
                                    onclick="batchUrl('<?php echo U('Home/Order/update',array('status'=>1));?>')">发货
                            </button>
                        </div>
                        <!--<div class="btn-group">-->
                            <!--<button type="button" class="btn btn-danger "-->
                                    <!--onclick="batchUrl('<?php echo U('Home/Order/update',array('pay_status'=>1));?>')">-->
                                <!--已支付-->
                            <!--</button>-->
                        <!--</div>-->
                        <div class="btn-group">
                            <button type="button" class="btn btn-danger "
                                    onclick="batchUrl('<?php echo U('Home/Order/update',array('status'=>-1));?>')">取消
                            </button>
                        </div>
                        <div class="btn-group">
                            <button type="button" class="btn btn-danger "
                                    onclick="batchUrl('<?php echo U('Home/Order/update',array('status'=>2));?>')">已完成
                            </button>
                        </div>
                        <div class="btn-group">
                            <button type="button" class="btn btn-danger "
                                    onclick="batchUrl('<?php echo U('Home/Order/update',array('status'=>-2));?>')">退货中
                            </button>
                        </div>
                        <div class="btn-group">
                            <button type="button" class="btn btn-danger "
                                    onclick="batchUrl('<?php echo U('Home/Order/update',array('status'=>-3));?>')">已退货
                            </button>
                        </div>
                        <div class="btn-group">
                            <button type="button" class="btn btn-danger "
                                    onclick="batchUrl('<?php echo U('Home/Order/export',array('status'=>I('get.status') , 'pay_status'=>I('get.pay_status') ,'day'=>I('get.day')));?>',false)">
                                导出订单
                            </button>
                        </div>
                    </div>
                    <div class="table-responsive" style="overflow-x: visible;">
                        <table class="table table-bordered table-hover">
                            ...
                            <?php if(is_array($orderList)): $i = 0; $__LIST__ = $orderList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?><tr>
                                    ...
                                    <td>
                                        <div>联系人:<span class="badge"><?php echo ($order["contact"]["name"]); ?></span></div>
                                        <div>手机号:<span class="badge"><?php echo ($order["contact"]["phone"]); ?></span></div>

                                    </td>
                                    <td style="max-width: 200px;overflow: hidden;">
                                        <div><?php echo ($order["contact"]["province"]); ?></div>
                                        <div><?php echo ($order["contact"]["city"]); ?></div>
                                        <div><?php echo ($order["contact"]["address"]); ?></div>
                                    </td>
                                    <td class="hidden-xs">
                                        <span style="background-color: #dd4b39;" class="badge"><?php echo ($order["totalprice"]); ?>元</span>
                                    </td>
                                    <td class="hidden-xs">
                                    ...
                                    <td class="hidden-xs">
                                        ...
                                        <?php $data = $order[detail]; for($i=0;$i < count($data);$i++){ echo '
