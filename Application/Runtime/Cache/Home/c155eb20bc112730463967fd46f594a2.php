...
<div class="login-box">
    ...
    <div class="login-box-body" style="margin-top:-65px">
        ...
        <form action="<?php echo U('Home/Public/register');?>" method="post">
            <div class="form-group has-feedback">
                <input type="text" class="form-control" placeholder="账号" onblur="checkName(this)" name="username" id="username" required>
...
<script>
    ...
    function checkName(obj){
        ...
        if($('#username').val()){
            var url = '<?php echo U("Home/Public/checkName");?>'+'/username/'+ $('#username').val();
            $.get(url, function (data) {
                if(data == 0){
                    layer.msg('名字已存在,请重新输入');
                    $(obj).focus();
                }else{
    ...
    function checkemail(obj){
        ...
        if($('#email').val()){
            // var url = '<?php echo U("Home/Public/checkemail");?>'+'/email/'+ $('#email').val();
            // $.get(url, function (data) {
            //     if(data == 0){
            //         layer.msg('该邮箱已经注册,请重新输入');
            //         $(obj).focus();
            //     }else{
            ...
            // });
            //向后台发送处理数据
            var url = '<?php echo U("Home/Public/checkemail");?>';
